(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('myHrAppConcediuApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-myHrAppConcediuApp-error');
                var entityKey = httpResponse.headers('X-myHrAppConcediuApp-params');
                if (errorHeader) {
